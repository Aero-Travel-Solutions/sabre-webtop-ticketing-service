    ...
    public class SabreGDS
    {
        ...
        private async Task<Agent> getAgentData(string sessionid, User user, string agentid, string webservicepcc)
        {
            ...
            Agent agent = new Agent()
            {
                ...
                PccList = (await _agentPccDataSource.RetrieveAgentPccs(user, sessionid)).PccList,
                AgentId = agentid,
                Agent = agentDetails,
        ...
        public async Task<IssueExpressTicketRS> IssueExpressTicket(IssueExpressTicketRQ request, string contextID)
        {
            ...
            try
            {
                ...
                //credit amount check
                var novalquotes = request.Quotes.IsNullOrEmpty() ? null: request.Quotes.Where(w => (w.BaseFare == w.EquivFare ? w.BaseFare : w.EquivFare) + w.TotalTax == 0.00M);
                ...
                agentpcc = (await getagentpccs(user, pcc.PccCode, request.SessionID)).FirstOrDefault();
        ...
        private async Task<List<string>> getagentpccs(User user, string pcc, string sessionId)
        {
            if (user == null)
            {
            ...
            var agentpcc = await _agentPccDataSource.RetrieveAgentPccs(user, sessionId);
            ...
            List<CurrencyData> currencydata, string pccdecimalformatstring)
        {
            ...
            foreach (var quotegrp in quotegroups)
            {
                ...
                if (quote.EquivFare != 0.00M && !string.IsNullOrEmpty(quote.EquivFareCurrencyCode) && quote.EquivFareCurrencyCode != quote.BaseFareCurrency)
                {
            ...
            if(quotes.All(a=> a.Route == quotes.First().Route))
            {
                ...
                var turnaroundpoint = _getTurnaroundPointDataSource.
                                                GetTurnaroundPoint(
                                                    ...
                                                    {
                                                        FareCalculation = quotes.First().FareCalculation,
                                                        Sectors = secs
                                                    }).
                                                    GetAwaiter().
                                                    GetResult();
