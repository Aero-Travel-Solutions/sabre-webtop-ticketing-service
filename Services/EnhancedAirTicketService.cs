                    ...
                    QuoteNos = quote.IsNullOrEmpty() ?
                                ...
                                quote.Select(s => new IssueTicketDocumentData()
                                {
                                    ...
                                    AgentPrice = s.QuotePassenger.FormOfPayment == null || s.QuotePassenger.FormOfPayment.PaymentType == PaymentType.CA ?
                                                        ...
                                                        s.QuotePassenger.FormOfPayment.PaymentType == PaymentType.CC && s.QuotePassenger.FormOfPayment.CreditAmount < ((s.EquivFare == s.BaseFare ? s.BaseFare : s.EquivFare) + s.TotalTax) ?
                                                            s.TotalFare - s.QuotePassenger.FormOfPayment.CreditAmount + s.Fee + (s.FeeGST.HasValue ? s.FeeGST.Value : 0.00M) - Math.Round((((s.EquivFare == s.BaseFare ? s.BaseFare : s.EquivFare) * s.AgentCommissionRate ?? 0.00M) / 100), 2, MidpointRounding.AwayFromZero) :
                                                            s.Fee + (s.FeeGST.HasValue ? s.FeeGST.Value : 0.00M) - Math.Round((((s.EquivFare == s.BaseFare ? s.BaseFare : s.EquivFare) * s.AgentCommissionRate ?? 0.00M) / 100), 2, MidpointRounding.AwayFromZero)
                                }).ToList(),
